...
if(Yii::$app->controller->action->id == 'index') $this->title = Yii::t('app','Disposal Instruction');
...
?>
<div id="southPanel" class="instruction-disposal-index">

	
	<div id="southPanelHeader">
	
        <?php
            if(Yii::$app->controller->action->id == 'index') {echo 'List Input Disposal Instruction';};
            if(Yii::$app->controller->action->id == 'indexverify') {echo 'List Planning BA Survey IKO Verification ';};
        ?>
		<?php if (Yii::$app->controller->action->id == 'index') { ?>
            <?= Html::a('Create', '#create', ['class' => 'btn btn-success headerButton pull-right', 'id' => 'createModal', 'value'=>Url::to(['ospm-grf/create']), 'header'=> yii::t('app','Create GRF Regular')]) ?>
        <?php } ?>
    </div>
