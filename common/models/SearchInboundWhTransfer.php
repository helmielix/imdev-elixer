    ...
    public function _search($params, $query)
    {
		...
		if ( isset($this->delta) ) {
            ...
            if ( $this->status_listing == 999 ){
            	$query  ->andFilterWhere(['inbound_wh_transfer.status_listing' => 50]);
                $query	->orWhere(['and', ['outbound_wh_transfer.status_listing' => 25], ['inbound_wh_transfer.status_listing' => null] ])
    					// ->andWhere()
    					;
            }else{
