...
use app\models\User;
use app\models\LogOsOutsourcePersonil;

/**
 * SearchOsOutsoucePersonil represents the model behind the search form about `app\models\OsOutsourcePersonil`.
 */
class SearchLogOsOutsourcePersonil extends LogOsOutsourcePersonil
{
    /**
     * @inheritdoc
     */

	public $overtime, $pph21, $position, $division, $id_division, $idCity;
    ...
    {
        $query = LogOsOutsourcePersonil::find()
	...
	public function searchByAction($params, $action)
    {
		...
		if($action == 'input') {
			$query->andFilterWhere(['or',['=','os_outsource_personil.status_listing', 1],['=','os_outsource_personil.status_listing', 3],['=','os_outsource_personil.status_listing', 2],['=','os_outsource_personil.status_listing', 7]])
            ...
            $query->andFilterWhere(['or',['=','log_os_outsource_personil.status_listing', 1],['=','log_os_outsource_personil.status_listing', 2],['=','log_os_outsource_personil.status_listing', 3],['=','log_os_outsource_personil.status_listing', 4],['=','log_os_outsource_personil.status_listing', 5],['=','log_os_outsource_personil.status_listing', 6],['=','log_os_outsource_personil.status_listing', 13]]);
        }
	...
	public function searchByIdDivisionIdRegion($params, $idpersonil)
	{
		...
		// $query->andFilterWhere(['os_outsource_personil.status_listing' => 5]);
		if (count($idpersonil) == 0) {
			$idpersonil = 0;
		}
		$query->andFilterWhere(['log_os_outsource_personil.id' => $idpersonil]);
			// ->orderBy(['os_outsource_personil.id' => SORT_DESC]);
...

    {
        ...
        $query->andFilterWhere(['log_os_outsource_parameter.id_division' => $idDivision]);

		$query->andFilterWhere(['log_os_outsource_personil.id_city' => $idCity]);

		$query->andFilterWhere(['log_os_outsource_salary.month' => $month]);
