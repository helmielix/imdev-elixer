...
use common\models\SearchDashGrfNewTask;
 ...
 */
class DashboardGrfController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    {	$task_date='';
        $task='';
        $table_source='';
        if (isset(Yii::$app->request->queryParams['SearchDashGrfNewTask'])) {
            $params = Yii::$app->request->queryParams['SearchDashGrfNewTask'];
            extract($params);
            // return "select action_dash_iko_new_task('$task_date','$table_source','$task','');";
        }
        $data = Yii::$app->db->createCommand("select _dash_grf_new_task('$task_date','$table_source','$task','');")->queryAll();

        $this->layout = 'map';
        $searchModel = new SearchDashGrfNewTask();
		...
		if(Yii::$app->user->can('/grf/indexverify')) {
			array_push($arrFilter,['ilike','table_source','GRF Verification']);
		}
		if(Yii::$app->user->can('/grf/indexapprove')) {
			array_push($arrFilter,['ilike','table_source','GRF Approval']);
		}
