...
<?php   function getFilterStatus() {
    ...
    if(Yii::$app->controller->action->id == 'indexotherslog')
        return [
            1 => 'Inputted',
            2 => 'Revised',
            3 => 'Need Revise',
    ...
    <div id="southPanelHeader">
        ...
        <?php
           ...
           if(Yii::$app->controller->action->id == 'indexlog')echo 'List Log History Good Request Form';
            if(Yii::$app->controller->action->id == 'indexotherslog')echo 'List Log History Good Request Form';
        ?>
		</h3>
		<div class="row">
			<div class="col-sm-12">
                    ...
                    'view' => function ($url, $model) {
                         if(Yii::$app->controller->action->id == 'index' && !isset($model->status_listing)){
                            return Html::a('<span style="margin:0px 2px" class="glyphicon glyphicon-eye-open"></span>', '#view?id='.$model->id.'&header=Detail_Material_GRF_Vendor_IKO', [
                                'title' => Yii::t('app', 'view'), 'class' => 'viewButton', 'value'=>Url::to(['grf/viewlog','id' => $model->id]), 'header'=> yii::t('app','Detail Material GRF Vendor IKO')  
                            ]);
                        } 
						else {
