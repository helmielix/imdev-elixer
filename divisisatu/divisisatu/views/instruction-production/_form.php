...
<div class="instruction-wh-transfer-form">
	...
	<?php echo ($model->isNewRecord) ? '' : $form->field($model, 'instruction_number')->textInput(['disabled' => true]) ?>

	   <?= $form->field($model, 'target_produksi')->widget(
      ...
      $dataItem = ItemProductionReference::find()->select(['distinct(item_hasil)','item_hasil_name'])->all();
  	?>
...
$('#createButton').click(function () {
    ...
    if (!form.find('.has-error').length) {
        ...
        data.append( 'file', $( '#instructionproduction-file' )[0].files[0] );
        var button = $(this);
