...
<div class="instruction-grf-form-detail">
    ...
    <?php $form = ActiveForm::begin([
		...
		<?php echo Html::hiddenInput('idGrf', Yii::$app->session->get('idGrf'));
         ?>
...
<script>
	...
	$('#createdButton').click( function(event){
		...
		if (!form.find('.has-error').length) {
			...
			data.append( 'InstructionGrfDetail[im_code]', $( '#instructiondetail-im_code' ).val() );
			data.append( 'InstructionGrfDetail[qty]', $( '#instructiondetail-qty' ).val() );
			...
			$.ajax({
				url: '<?php echo Url::to(['/instruction-grf/'.$this->context->action->id,'orafinCode'=>$modelOrafin->orafin_code,'rrNumber'=>$modelOrafin->rr_number, 'maxQty' => $modelOrafin->qty]) ;?>',
				type: 'post',
				data: data,
				cache: false,
				processData: false,
				...
				success: function (response) {
					if(response == 'success') {
						...
						// // }else{
							$('#modal').modal('show')
							 .find('#modalContent')
							 .load('<?php echo Url::to(['/instruction-grf/indexdetail']) ;?>');
							$('#modalHeader').html('<h3> Instruction Grf Detail </h3>');
