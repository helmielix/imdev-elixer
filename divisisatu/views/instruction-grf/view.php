...
<div class="instruction-grf-view">    
    ...
    <p> 
        ...
        <?php if((Yii::$app->controller->action->id == 'viewapprove' && $model->status_listing != 5) || 
                (Yii::$app->controller->action->id == 'viewinstruction' && $model->status_listing == 5) || (Yii::$app->controller->action->id == 'viewverify' && $model->status_listing != 4))
            echo Html::button(Yii::t('app','Revise'), ['id'=>'reviseButton','class' => 'btn btn-warning']); ?>
         <?php if(Yii::$app->controller->action->id == 'viewverify' && $model->status_listing != 4 && $model->status_listing != 4)
...
$('#verifyButton').click(function () {
        $.ajax({
            url: '<?php echo Url::to(['/instruction-grf/verify', 'id' => $model->id]) ;?>',
            type: 'post',
            success: function (response) {
                $('#modal').modal('hide');
