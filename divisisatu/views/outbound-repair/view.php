...
<div class="outbound-wh-transfer-view">
	...
	<p> 
        ...
        <?php if((Yii::$app->controller->action->id == 'viewapprove' && $model->status_listing != 5 && $model->status_listing!=22))
    ...
    <div class="form-group field-instructionwhtransfer-revision_remark">
        ...
        <div class="col-sm-6">
<?= Html::textArea('OutboundRepair[revision_remark]', '', ['rows' => '5', 'class' => 'form-control', 'id' => 'instruction-wh-transfer-revision_remark']) ?>
        </div>
    </div>
    <br />
...
$('#submitButton').click(function () {
	...
	var form = $('#createForm');
   	...
   	if (!form.find('.has-error').length) {
		...
		data.append('OutboundRepairTransfer[file_attachment]', $('input[type=file]')[0].files[0]);

		if(data['OutboundRepairTransfer[file_attachment]']) {
			console.log('sdfdsf');
		} else {
			console.log('salah');
		}
  ...
  $('#revisionButton').click(function () {
        ...
        data = new FormData();
        data.append('OutboundRepair[revision_remark]', $('#instruction-wh-transfer-revision_remark').val());
        var button = $(this);
        button.prop('disabled', true);
        button.append(' <i id="spinRefresh" class="fa fa-spin fa-refresh"></i>');

        $.ajax({
            url: '<?= Url::to([$this->context->id . '/reviseoutbound', 'id' => $model->id_instruction_repair]) ?>',
            type: 'post',
            data: data,
            processData: false,
            contentType: false,
    ...
    $('#approveButton').click(function () {
        ...
        $.ajax({
            url: '<?php echo Url::to([$this->context->id . '/approve', 'id' => $model->id_instruction_repair]); ?>',
            type: 'post',
            success: function (response) {
                $('#modal').modal('hide');
