                    ...
                    'create' => function ($url, $model) {
                        if($model->status_tagsn == 999 || $model->status_tagsn == 44){
                            ...
                            return Html::a('<span style="margin:0px 2px;" class="label label-success">'.$label.'</span>', '#', [
                                'title' => Yii::t('app', 'upload'), 'class' => 'viewButton', 'value'=> Url::to([$this->context->id.$link, 'id' => $model->id_inbound_detail, 'idInboundWh' => Yii::$app->session->get('idInboundWh')]), 'idInboundWh' => $model->id_inbound_detail, 'header'=> yii::t('app','Create Material GRF Vendor IKO')
                            ]);
                        }
                    },
