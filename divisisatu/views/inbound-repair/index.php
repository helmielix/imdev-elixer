<?php

use yii\helpers\Html;
use kartik\grid\GridView;
use yii\widgets\Pjax;
use yii\bootstrap\Modal;
use yii\helpers\Url;
use yii\helpers\ArrayHelper;
use dosamigos\datepicker\DatePicker;
use kartik\export\ExportMenu;
use common\models\StatusReference;

$this->title = Yii::t('app','Repair Instruction');
// if(Yii::$app->controller->action->id == 'index')
// if(Yii::$app->controller->action->id == 'indexapprove') $this->title = Yii::t('app','Verify GRF Vendor IKO');

$this->registerJsFile('@commonpath/js/btn_modal.js',['depends' => [\yii\web\JqueryAsset::className()]]);
$this->registerJsFile('@commonpath/js/popup_alert.js',['depends' => [\yii\web\JqueryAsset::className()]]);
//print_r($dataProvider->getModels());exit();

function getFilterStatus() {
    if(Yii::$app->controller->action->id == 'index')
        return [
            46 => 'Completed',
            47 => 'Uncompleted',
            45 => 'New Intransit',
        ];
    else if(Yii::$app->controller->action->id == 'indexsn')
        return [
            1 => 'Inputted',
            49 => 'New Inbound',
        ];
};

function getStatusListing() {
    if (Yii::$app->controller->action->id == 'index' || Yii::$app->controller->action->id == 'indexsn') {
        return 'status_listing';
    } else {
        return 'inbound_repair_tagging.status_listing';
    }
};

?>
<?php Modal::begin([
    'header'=>'<h3 id="modalHeader"></h3>',
    'id'=>'modal',
    'size'=>'modal-lg',

]);

echo '<div id="modalContent"> </div>';

Modal::end();
?>

<div>
    <?php
    Modal::begin([
        'header' => '<h4>Modal SN</h4>',
        'id' => 'modal2',
        'size' => 'modal-lg',
    ]);

    echo "<div id='modalContent2'></div>";

    Modal::end();
    ?>
</div>

<div id="southPanel">

    <div id="southPanelHeader">
        <div class="panelZoomController">

        </div>
        <h3>
            <?php
            if(Yii::$app->controller->action->id == 'index'){
                echo 'List Input Inbound Repair';
            };
            if(Yii::$app->controller->action->id == 'indexsn'){echo 'List Tag SN';};
            ?>
        </h3>

    </div>




    <?php Pjax::begin(['id' => 'pjax', 'timeout' => false, 'enablePushState' => false, 'clientOptions' => ['method' => 'POST']]) ?>
    <?= GridView::widget([
        'dataProvider' => $dataProvider,
        'filterModel' => $searchModel,
        'columns' => [
            ['class' => 'kartik\grid\SerialColumn'],
            [
                'class' => 'yii\grid\ActionColumn',
                'template'=>'{view}',
                'buttons'=>[
                    'view' => function ($url, $model) {
                        if(Yii::$app->controller->action->id == 'index' && $model->status_listing==45){
                            //print_r($model->id_instruction_repair);
                            return Html::a('<span style="margin:0px 2px" class="glyphicon glyphicon-plus"></span>', '#input?id='.$model->id_instruction_repair.'&header=input_inbound_repair', [
                                'title' => Yii::t('app', 'view'), 'class' => 'viewButton', 'value'=>Url::to([$this->context->id.'/input','id' => $model->id_instruction_repair]), 'header'=> yii::t('app','List Input Inbound')
                            ]);
                        }
                        if(Yii::$app->controller->action->id == 'indexsn'){
                            //print_r($model->id_instruction_repair);
                            return Html::a('<span style="margin:0px 2px" class="glyphicon glyphicon-plus"></span>', '#createtag?id='.$model->id_instruction_repair.'&header=Create_Tag_SN', [
                                'title' => Yii::t('app', 'view'), 'class' => 'viewButton', 'value'=>Url::to([$this->context->id.'/createtag','id' => $model->id_instruction_repair]), 'header'=> yii::t('app','Create Tag SN')
                            ]);
                        }
                        else {
                            if(Yii::$app->controller->action->id == 'index') {
                                $viewurl = 'viewlist';
                                $header = 'List Input Inbound';
                            }

                            if(Yii::$app->controller->action->id == 'tagsn') {
                                $viewurl = 'viewtagsn';
                                $header = 'List Tag SN';
                            }
                            if(Yii::$app->controller->action->id == 'indexoverview'){
                                $viewurl = 'viewoverview';
                            } else {
                                $header = 'List Input Inbound';
                                $viewurl = 'viewlist';
                            }

                            $headerlnk = str_replace(' ', '_', $header);
                            return Html::a('<span style="margin:0px 2px" class="glyphicon glyphicon-eye-open"></span>', '#'.$viewurl.'?id='.$model->id_instruction_repair.
                                '&header='.$headerlnk, [
                                'title' => Yii::t('app', 'view'), 'class' => 'viewButton', 'value'=>Url::to([$this->context->id.'/'.$viewurl, 'id' => $model->id_instruction_repair]), 'header'=> yii::t('app',$header)
                            ]);
                        }
                    },
                ],
            ],
            [
                'attribute' => getStatusListing(),
                'format' => 'raw',
                'label' => 'Status',
                'value' => function ($searchModel) {

                    if (Yii::$app->controller->action->id == 'index') {
                        if ($searchModel->status_listing) {
                            return "<span class='label' style='background-color:{$searchModel->statusReference->status_color}' >{$searchModel->statusReference->status_listing}</span>";
                        } else {
                            return "<span class='label' style='background-color:red'>New Instruction</span>";
                        }
                    }else if (Yii::$app->controller->action->id == 'indexsn') {
                        if ($searchModel->status_listing!=46) {
                            return "<span class='label' style='background-color:{$searchModel->statusReference->status_color}' >{$searchModel->statusReference->status_listing}</span>";
                        } else {
                            return "<span class='label' style='background-color:red'>New Instruction</span>";
                        }
                    } else {
//                        if ($searchModel->tagging) {
                        if ($searchModel->status_listing) {
//                            echo "<pre>";print_r($searchModel);
                            return "<span class='label' style='background-color:{$searchModel->statusReference->status_color}' >{$searchModel->statusReference->status_listing}</span>";
                        } else {
                            return "<span class='label' style='background-color:red'>New Instruction</span>";
                        }
                    }
                },
                'filter' => getFilterStatus()
            ],
            [
                'attribute' => 'no_sj',
                //'visible' => $this->context->action->id == 'indexprintsj',
            ],
            [
                'attribute' => 'instruction_number',
                'visible' => false,
            ],
            [
                'attribute' => 'tanggal_datang',
                //'label' => 'Tanggal Datang',
                /*'value' => function($model) {
                            return 'Aug 21, 2018';
                },*/
                'format' => 'date',
                'filter' => DatePicker::widget([
                    'model' => $searchModel,
                    'attribute' => 'tanggal_datang',
                    'clientOptions' => [
                        'autoclose' => true,
                        'format' => 'yyyy-mm-dd',
                    ],
                ]),
            ],
            [
                'attribute' => 'vendor',
                'label' => 'Vendor',
                'value' => function($model) {
                    //print_r($model);
                    return $model->instructionRepair->idVendor->name;
                },
            ],
            /*[
                'attribute' => 'name',
                //'label' => 'Vendor',
                'value' => function($model) {
                    return 'ELIXIR';
                },
            ],*/
            /*[
                'attribute' => 'created_date',
                'value'  => 'created_date',
                'format' => 'datetime',
                'filter' => DatePicker::widget([
                    'model' => $searchModel,
                    'attribute' => 'created_date',
                    'clientOptions' => [
                        'autoclose' => true,
                        'format' => 'yyyy-mm-dd',
                    ],
                ]),
            ],
            [
                'attribute' => 'updated_date',
                'value'  => 'updated_date',
                'format' => 'datetime',
                'filter' => DatePicker::widget([
                    'model' => $searchModel,
                    'attribute' => 'updated_date',
                    'clientOptions' => [
                        'autoclose' => true,
                        'format' => 'yyyy-mm-dd',
                    ],
                ]),
            ],*/
        ],
    ]); ?>
    <?php Pjax::end(); ?></div>

<script>
    $('.condButton').click(function () {
        //alert('HAI!')
        //$('#modal').modal('hide')
        $('#modal2').modal('show')
            .find('#modalContent2')
            .load('<p>tes</p>');
        //$('#modalHeader2').html('<h3> Update Warehouse Transfer Instruction</h3>');
    });
</script>