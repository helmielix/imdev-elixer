...
$this->title = $model->id;
$this->params['breadcrumbs'][] = ['label' => 'Instruction Repair', 'url' => ['index']];
...
<div class="instruction-wh-transfer-view">    
    <div class="row">
            ...
            DetailView::widget([
                ...
                'attributes' => [
                    ...
                    [ 'label' => 'Tanggal Pengiriman',
                        'value'=> $model->target_pengiriman,
                        'format'=>"date"
                        ]
                   ,
                    'idWarehouse.nama_warehouse',
                    'idVendor.name',
                    'note',
                // 'wh_destination',
                ],
        ...
        <!--		<div class="col-sm-12">
<?php if ($model->status_listing != 6) {
            //echo 'hai!';
            echo Html::button(Yii::t('app', 'Update'), ['id' => 'updateButton', 'class' => 'btn btn-primary']);
    }

?>
<?php if (($model->status_listing == 1 || $model->status_listing == 6 || $model->status_listing == 7))
...
?>
    ...
    <!--<h3>Detail Repairs Instruction</h3>-->
    <?=
    $this->render('indexdetail', [
        'model' => $model,
        'searchModel' => $searchModel,
        'dataProvider' => $dataProvider,
        'disablebtn'=>$disableaddbtn,
    ])
    ?>
...
//            echo Html::button(Yii::t('app','Reject'), ['id'=>'rejectButton','class' => 'btn btn-danger']); 
                ...
                ?>
        <?php
        if (Yii::$app->controller->action->id == 'viewapprove' && $model->status_listing != 5)
              echo Html::button(Yii::t('app', 'Revise'), ['id' => 'reviseButton', 'class' => 'btn btn-warning', 'style' => 'margin-top: 5px;']);
                if(Yii::$app->controller->action->id == 'viewinstruction' && $model->status_listing == 5)
            echo Html::button(Yii::t('app', 'Report To IC'), ['id' => 'reviseButton', 'class' => 'btn btn-warning']);
        ?>
        <?php if (Yii::$app->controller->action->id == 'viewapprove' && $model->status_listing != 5)
            echo Html::button(Yii::t('app', 'Approve'), ['id' => 'approveButton', 'class' => 'btn btn-success', 'style' => 'margin-top: 5px;']);
        ?>
