...
<div class="instruction-wh-transfer-form">
    ...
    <?= $form->field($model->idMasterItemIm, 'im_code')->textInput(['disabled' => true]) ?>
<?= $form->field($modelItem, 's_reject')->textInput(['disabled' => true]) ?>
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response == 'success') {
                        $('#modal').modal('show')
                                .find('#modalContent')
                                .load('<?php echo Url::to([$this->context->id . '/indexdetail', 'id' => Yii::$app->session->get('idInstRep')]); ?>');
