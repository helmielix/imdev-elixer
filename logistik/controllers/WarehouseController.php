...
class WarehouseController extends Controller
{
	...
	public function actionUserwarehouse($idwarehouse){
		...
		if (Yii::$app->request->isPost) {
            ...
            // remove akses semua WH
            $warehouse = Warehouse::findOne($idwarehouse);
            $whpusat = false;
            if($warehouse->wh_type == 17){
                $whpusat = true;
                User::updateAll(['wh_level' => 2]); // rubah semua akses user terhadap WH
            }
			...
			foreach($iduser as $id_user){
                ...
                if($whpusat){
                    ...
                    $user->wh_level = 1; // bisa akses semua WH
                    $user->save(false);
                }
			}
