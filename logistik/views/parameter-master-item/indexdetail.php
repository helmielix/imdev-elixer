        ...
        'columns' => [
            ...
            [
                'value' => function($model){
                    if($model->masterItemChild) return $model->masterItemParent->name;
                },
                'label' => 'Name'
            ]   ,
            [
                'value' => function($model){
                    if($model->masterItemChild) return $model->masterItemParent->im_code;
                },
                'label' => 'IM Code'
            ]		,
            'qty_item_child'
...
<script>
    ...
    $('#createButton').click(function () {
        ...
        $('#modalHeader').html('<h3> Create Detail</h3>');
    });
    ...
    $('#submitButton').click(function () {
		...
		$.ajax({
            url: '<?php echo Url::to([$this->context->id.'/submit', 'id' => Yii::$app->session->get('idParameter')]) ;?>',
            type: 'post',
            processData: false,
            contentType: false,
