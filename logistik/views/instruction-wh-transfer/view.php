...
<div class="instruction-wh-transfer-view">
	...
	<div class="row">
		<div class="col-sm-6">
			<?= DetailView::widget([
				...
				'template' => '<tr><th width="50%" {captionOptions}>{label}</th><td{contentOptions}>{value}</td></tr>',
				'attributes' => [
					'instruction_number',
					'delivery_target_date:date',
					[
		            	'attribute' => 'wh_origin',
		            	'value' => $model->whOrigin->nama_warehouse
		            ],
		            [
		            	'attribute' => 'wh_destination',
		            	'value' => $model->whDestination->nama_warehouse
		            ],
				],
			]) ?>
		</div>
			...
			<?= DetailView::widget([
				...
				'attributes' => [
					...
					[
						...
						'visible' => $this->context->action->id == 'viewinstruction',
					],
					[
	...
	<div class="col-sm-12">
		<?php if((Yii::$app->controller->action->id == 'view' || Yii::$app->controller->action->id == 'deletedetail') && $model->status_listing != 6)
			echo Html::button(Yii::t('app','Update'), ['id'=>'updateButton','class' => 'btn btn-primary']); ?>
		<?php if((Yii::$app->controller->action->id == 'view' || Yii::$app->controller->action->id == 'deletedetail') && ($model->status_listing == 1 || $model->status_listing == 6 || $model->status_listing == 7))
	...
	<div class='col-sm-12'>
		...
		<?php if((Yii::$app->controller->action->id == 'viewinstruction' && $model->status_listing == 5))
            echo Html::button(Yii::t('app','Input Tag SN'), ['id'=>'createoutButton','class' => 'btn btn-success']); ?>
