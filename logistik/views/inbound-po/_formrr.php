		...
		'requiredCssClass' => 'requiredField'
			...
			<?php
				...
				if (Yii::$app->controller->action->id == 'update') {
					...
					$depdropUnit['data'] = ArrayHelper::map($out, 'name', 'name');
				}
		    ...
		    <?= $form->field($model, 'id_warehouse')->widget(Select2::classname(), [
		        ...
		        'language' => 'en',
		        'options' => ['placeholder' => 'Select '.$model->getAttributeLabel('id_warehouse')],
		        'pluginOptions' => [
		        'allowClear' => true],
		        ]) ?>
	...
	$('#createButton').click(function (event) {
		...
		if (!form.find('.has-error').length) {
			data = new FormData(form[0]);
       		...
       		data.append( 'file', $( '#inboundpo-file' )[0].files[0] );
            var button = $(this);
