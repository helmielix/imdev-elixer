			...
			<div class="col-sm-12">
				<p class="pull-right">
					<?php if (Yii::$app->controller->action->id == 'index') { ?>
						<?=  Html::a('Create', '#create?header=Create Instruksi Warehouse Transfer', ['class' => 'btn btn-success', 'id' => 'createModal', 'value'=>Url::to(['instruction-repair/create']), 'header'=> yii::t('app','Create Instruksi Warehouse Transfer')]) ; ?>
					<?php } ?>
				</p>
			</div>
		</div>
                    ...
                    'view' => function ($url, $model) {
                         if(Yii::$app->controller->action->id == 'index' && isset($model->status_listing)){
                            return Html::a('<span style="margin:0px 2px" class="glyphicon glyphicon-eye-open"></span>', '#view?id='.$model->id.'&header=Detail_Material_GRF_Vendor_IKO', [
                                'title' => Yii::t('app', 'view'), 'class' => 'viewButton', 'value'=>Url::to(['instruction-repair/view','id' => $model->id]), 'header'=> yii::t('app','Detail Material GRF Vendor IKO')
                            ]);
                        }
						else {
                            ...
                            return Html::a('<span style="margin:0px 2px" class="glyphicon glyphicon-eye-open"></span>', '#'.$viewurl.'?id='.$model->id.'&header=Detail_Material_GRF_Vendor_IKO', [
                                'title' => Yii::t('app', 'view'), 'class' => 'viewButton', 'value'=>Url::to(['instruction-repair/'.$viewurl, 'id' => $model->id]), 'header'=> yii::t('app','Detail Inbound PO')
                                ]);
                        }
                    },
                ],
