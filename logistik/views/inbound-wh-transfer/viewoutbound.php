...
<div class="outbound-wh-transfer-view">
	...
	<p> 
        <?php if(Yii::$app->controller->action->id == 'viewoutbound' )
            echo Html::button(Yii::t('app','Input Inbound'), ['id'=>'createButton','class' => 'btn btn-success']); ?>
...
<script>
	...
	$('#createButton').click(function () {
		<?php 
			if($modelInbound == 'create'){
				$url = Url::to([$this->context->id.'/create', 'idOutboundWh' => $model->id_instruction_wh]);
			}else{
				$url = Url::to([$this->context->id.'/update', 'idOutboundWh' => $model->id_instruction_wh]);
			}
		?>
		$('#modal').modal('show')
			.find('#modalContent')
