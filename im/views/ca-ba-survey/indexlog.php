...
if(Yii::$app->controller->action->id == 'indexlog') $this->title = Yii::t('app','Log History BA Survey');
...
<div id="southPanel" > 
	<div id="southPanelHeader">
		...
		<?php
            if(Yii::$app->controller->action->id == 'indexlog') {echo 'List Log History BA Survey';};
        ?>
	</div>
				...
				[
					...
					'buttons' => [
						'view' => function ($viewurl, $model) {
									...
									return Html::a('<span style="margin:0px 2px" class="glyphicon glyphicon-eye-open"></span>', '#'.$viewurl.'?id='.$model->idlog.'&header=Detail_Berita_Acara_Survey', [
											'title' => Yii::t('app', 'view'), 'class' => 'viewButton', 'value'=>Url::to(['/ca-ba-survey/'.$viewurl, 'id' => $model->idlog]), 'header'=> yii::t('app','Detail Berita Acara Survey')
								]);
						},
			...
			array_push($columns,
				[
					...
					'value' => function ($searchModel) {
						if($searchModel->statusReferenceListing){
