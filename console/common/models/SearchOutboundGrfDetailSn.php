...
/**
 * SearchOutboundGrfDetailSn represents the model behind the search form about `common\models\OutboundGrfDetailSn`.
 */
class SearchOutboundGrfDetailSn extends OutboundGrfDetailSn
{
    /**
     * @inheritdoc
     */
    ...
    {
        return [
            [['id', 'id_outbound_grf_detail', 'condition'], 'integer'],
     ...
     */
    public function search($params, $idOutboundGrfDetail){
        $query = OutboundGrfDetailSn::find();
        $query->select([ 
            'outbound_grf_detail_sn.serial_number',
            'outbound_grf_detail_sn.condition',
            'outbound_grf_detail_sn.mac_address',
        ]);
        $query->andWhere(['id_outbound_grf_detail' => $idOutboundGrfDetail]);
    ...
    public function _search($params, $query)
    {
		...
		switch ($this->condition){
			...
			break;
            case 6: //not good dismantle
                $query->andFilterWhere(['condition' => 'good recondition']);
            break;
		}
