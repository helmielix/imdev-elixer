                ...
                'buttons'=>[
                    'view' => function ($url, $model) {
                         if(Yii::$app->controller->action->id == 'index' && (!isset($model->status_listing))){
                            return Html::a('<span style="margin:0px 2px" class="glyphicon glyphicon-eye-open"></span>', '#viewoutbound?id='.$model->id_outbound_wh.'&header=Detail_Material_GRF_Vendor_IKO', [
                                'title' => Yii::t('app', 'view'), 'class' => 'viewButton', 'value'=>Url::to(['inbound-wh-transfer/viewoutbound','id' => $model->id_outbound_wh]), 'header'=> yii::t('app','Detail Inbound WH Transfer')
                            ]);
                        } 
						else {
