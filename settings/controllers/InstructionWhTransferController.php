	...
	public function actionCreatedetail(){
		...
		if (Yii::$app->request->isPost && empty(Yii::$app->request->post('SearchMasterItemIm'))){
			...
			foreach($data_im_code as $key => $value){
				// if($data_r_good[$key] == '')
				if($data_r_good[$key] == '' && $data_r_notgood[$key] == '' && $data_r_reject[$key] == '' && $data_r_good[$key] == 0 && $data_r_notgood[$key] == 0 && $data_r_reject[$key] == 0 && $data_r_good_dis[$key] == '' && $data_r_notgood_dis[$key] == ''){
					continue;
				}
				...
				$model->req_good_dismantle		= ($data_r_good_dis[$key] == '') ? 0 : $data_r_good_dis[$key];
				$model->req_not_good_dismantle	= ($data_r_notgood_dis[$key] == '') ? 0 : $data_r_notgood_dis[$key];
				...
				if($model->req_good_dismantle > $modelMasterItem->s_good_dismantle){
					$pesan[] = $model->getAttributeLabel('req_good_dismantle')." is more than Stock for IM Code ".$values[1];
					$overStock = 0;
				}
				if($model->req_not_good_dismantle > $modelMasterItem->s_not_good_dismantle){
        ...
        $dataProvider = $searchModel->searchByCreateDetailItem(Yii::$app->request->post(), $modelInstruction->wh_origin, $idItemIm);
